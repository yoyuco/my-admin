<template>
  <div class="min-h-screen flex items-center justify-center bg-neutral-50 p-4">
    <div class="w-full max-w-md">
      <h1 class="text-center text-2xl font-semibold tracking-tight mb-6">Welcome Gege Team!</h1>
      ...
      <n-card :bordered="false" class="shadow-sm">
        ...
        >
          ...
          <n-button type="primary" block :loading="loading" @click="handleSignup">
            Tạo tài khoản
          </n-button>
        </n-form>
      </n-card>
...
async function resetPassword() {
  ...
  try {
    ...
    const { error } = await supabase.auth.resetPasswordForEmail(signin.value.email.trim(), {
      redirectTo,
    })
    if (error) throw error
