      ...
      <n-drawer-content title="Chi tiết log">
        <template v-if="current">
          <div class="grid grid-cols-2 gap-2 mb-3">
            <div class="meta">ID</div><div class="val">{{ current.id }}</div>
            <div class="meta">Thời điểm</div><div class="val">{{ dt(current.at) }}</div>
            <div class="meta">Actor</div><div class="val mono">{{ current.actor || '—' }}</div>
            <div class="meta">Entity</div><div class="val">{{ current.entity }}</div>
            <div class="meta">Table</div><div class="val">{{ current.table_name }}</div>
            ...
            <div class="meta">Action</div><div class="val">{{ current.action || '—' }}</div>
          </div>
...
import {
  NCard, NButton, NDataTable, NSwitch, NTag, NTooltip, NDrawer, NDrawerContent, DatePickerProps,
  NDivider, NInput, NDatePicker, NSelect, createDiscreteApi, type DataTableColumns, NAlert
} from 'naive-ui';
...
function startPoll(){ stopPoll(); poll = window.setInterval(() => { if (autoRefresh.value && canView.value) loadLogs() }, 30000) }
...
async function loadLogs() {
  ...
  loading.value = true
  try {
    const q = supabase.from('audit_logs')
      .select('id, at, actor, action, entity, entity_id, table_name, op, pk, diff, row_old, row_new, ctx', { count: 'exact' })
...
/* ======== Table columns ======== */
function dt(iso: string){ try { return new Date(iso).toLocaleString() } catch { return iso } }
...
const columns: DataTableColumns<LogRow> = [
  ...
  { title: 'Op', key: 'op', width: 90, render: (row) => h(NTag, { size:'small', type: opTag(row.op).type, bordered:false }, { default: () => opTag(row.op).label }) },
  ...
  },
  {
    ...
    width: 110,
    align: 'center',
    render: (row) => h(NButton, { size:'tiny', tertiary: true, onClick: () => openDetail(row) }, { default: () => 'Xem' })
  }
]
function safeClip(s: string, n = 220){ if (!s) return '—'; return s.length > n ? s.slice(0, n-1) + '…' : s }
function pretty(v: any){ try { return JSON.stringify(v ?? null, null, 2) } catch { return String(v) } }

/* ======== Drawer detail ======== */
const detailOpen = ref(false);
const current = ref<LogRow | null>(null);
function openDetail(row: LogRow){ current.value = row; detailOpen.value = true }
...
function exportCSV() {
  const headers = ['id','at','actor','entity','table_name','op','action','pk','diff','row_old','row_new','ctx']
