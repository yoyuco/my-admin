...
BEGIN
  -- Bước 1: Lấy order_id và ngữ cảnh từ order_line_id
  SELECT 
    ol.order_id,
  ...
  -- Bước 3: Cập nhật trạng thái và lưu bằng chứng
...
BEGIN
    ...
    -- <<< SỬA LỖI Ở ĐÂY: Dùng get_current_profile_id() để so sánh >>>
    ...
    -- Phần logic còn lại của hàm giữ nguyên
    v_order_line_id := v_session.order_line_id;
    SELECT ol.order_id, 
           (SELECT a.code FROM public.product_variant_attributes pva JOIN public.attributes a ON pva.attribute_id = a.id WHERE pva.variant_id = ol.variant_id AND a.type = 'SERVICE_TYPE' LIMIT 1) 
    INTO v_order_id, v_service_type 
    FROM public.order_lines ol WHERE ol.id = v_order_line_id;

    IF v_service_type = 'SELFPAY' AND v_session.unpaused_duration IS NOT NULL THEN 
        UPDATE public.order_lines 
        SET deadline_to = deadline_to - v_session.unpaused_duration, 
    ...
    IF completed_sessions_count > 0 THEN 
        ...
        ELSIF v_service_type = 'SELFPAY' THEN 
            ...
            UPDATE public.order_lines SET paused_at = NOW() WHERE id = v_order_line_id AND paused_at IS NULL; 
        END IF;
    ELSE 
...
DECLARE
  target_order_id uuid;
  v_context jsonb; -- Biến để lưu ngữ cảnh
BEGIN
  -- Bước 1: Lấy order_id và ngữ cảnh từ p_line_id
  SELECT 
    ol.order_id,
    jsonb_build_object('game_code', o.game_code, 'business_area_code', 'SERVICE')
  INTO 
...
using (((bucket_id = 'work-proofs'::text) AND has_permission('admin:manage_roles'::text)));
  ...
  create policy "Allow authenticated inserts for raw-proofs muto1g_0"
  on "storage"."objects"
  as permissive
  for insert
  to public
with check (((bucket_id = 'raw-proofs'::text) AND (auth.role() = 'authenticated'::text)));
